...
def parse_dns_response(response):
    ...
    _, flags, qd_count, an_count, _, _ = struct.unpack('!HHHHHH', dns_header)

    # Extract answers section
    answer_section = response[12:]

    # Initialize variables to store DNS records
    dns_records = []

    # Parse DNS answers
    for _ in range(an_count):
        name, answer_type, _, _, answer_data = parse_dns_answer(answer_section)
        dns_records.append({'name': name, 'type': answer_type, 'data': answer_data})

    return dns_records

def parse_dns_answer(answer_section):
    # Implement the logic to parse a DNS answer
    # This should extract the name, type, and data from the answer_section
    # Update this part based on the DNS protocol specifications

    # Placeholder values, replace with actual implementation
    name = "example.com"
    answer_type = "A"
    answer_data = "127.0.0.1"

    return name, answer_type, 0, 0, answer_data
...
if __name__ == "__main__":  
    ...
    print("RTT1 (First DNS Request): ", rtt)
    ...
    end_time = time.time()
    rtt = end_time - start_time
    print("RTT2 (Second DNS Request): ", rtt)
    ...
    # Parse DNS response
    dns_records = parse_dns_response(ipv4_dns_response)

    # Display DNS records
    for record in dns_records:
        print(f"Name: {record['name']}, Type: {record['type']}, Data: {record['data']}")
